...
// Pesquisa em tempo real no Spotify, YouTube e TikTok
// ============================================================
...
import { executeUniversalSearch, type SearchTrackItem } from '@/api/universalSearchService'
...
type TabType = 'all' | 'spotify' | 'youtube' | 'tiktok'
...
export function Search({ isAuthenticated = false }: SearchProps) {
  ...
  const { customPlaylists, addSpotifyItem } = useLibraryStore()
  ...
  // Universal Search Query (Spotify, YouTube e TikTok)
    ...
    <div className="space-y-8 max-w-7xl mx-auto select-none pb-20">
      ...
      <div className="max-w-2xl mx-auto space-y-4 pt-4">
        ...
        <p className="text-white/40 text-xs text-center">
          Busque simultaneamente no catálogo do <span className="text-spotify-green font-bold">Spotify</span>, <span className="text-youtube-red font-bold">YouTube</span> e <span className="text-tiktok-pink font-bold">TikTok</span>
        </p>
        ...
        <div className="relative">
          ...
          <input
            ...
            placeholder="Digite o nome da música, artista, canal ou cole um link..."
      ...
      {debouncedQuery && (
        <div className="space-y-10">
          ...
          {/* ─── TAB: TUDO (TODAS AS PLATAFORMAS COMBINADAS) ─── */}
          {activeTab === 'all' && searchData && (
            <div className="space-y-10">
              {/* Spotify Section */}
              {searchData.spotifyTracks.length > 0 && (
                <section className="space-y-4">
          ...
          {!isLoading && totalResults === 0 && (
            <div className="p-12 text-center rounded-3xl bg-white/[0.02] border border-white/5 space-y-3">
              ...
              <p className="text-white/40 text-xs">Tente buscar por outro termo, artista ou cole o link direto.</p>
            </div>
          )}
        </div>
      )}
